...
module.exports = {
    sailient: function() {
        return db.load(`SELECT * FROM posts where Status =0 and Premium=0 and DATEDIFF(CURDATE(), posts.Date)<100 GROUP by Conf ORDER BY Conf DESC LIMIT 2`);
    },
    mostview: function() {
        return db.load(`SELECT * FROM posts where Status =0 and Premium=0
            GROUP by Conf 
            ORDER BY Conf DESC
            LIMIT 10`);
    },
    newpost: function() {
        return db.load(`SELECT * FROM posts where Status =0 and Premium=0 ORDER BY Date DESC LIMIT 11`);
    },
    newpostbycat: function() {
        return db.load(`SELECT * FROM posts where Status =0 and Premium=0 GROUP by Category ORDER BY Date DESC`);
    },
